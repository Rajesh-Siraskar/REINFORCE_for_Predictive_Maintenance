...
def compute_metrics_simple(df):
    metrics = df.groupby(['Algorithm']).agg({'Wtd_Precision': ['mean'], 'Wtd_Recall': ['mean'], 'F1_Beta': ['mean'],
...
def test_script(training_round, df, algo, episodes, env, env_info, agent, test_cases, training_info, data_file, wear_threshold, results_file):
    dt = datetime.datetime.now()
    dt_d = dt.strftime('%d-%b-%Y')
    dt_t = dt.strftime('%H:%M:%S')
